...
class NewDROMatDataset(Dataset):
    ...
    def _collect_sample_ids(self) -> list[str]:
        dro_files = glob.glob(os.path.join(self.root_dir, "sample_*_dro.mat"))
        if not dro_files:
            raise FileNotFoundError(f"No DRO .mat files found in {self.root_dir}.")
        available = sorted({os.path.basename(fp).replace("_dro.mat", "") for fp in dro_files})
    ...
    def _load_dro_mat(self, sample_id: str):
        dro_path = os.path.join(self.root_dir, f"{sample_id}_dro.mat")
...
def main():
    ...
    traj_mat_path = os.path.join(
        dro_dataset_root, f"{traj_sample_id}_kspace_{int(N_spokes_eval)}spf.mat"
    )
